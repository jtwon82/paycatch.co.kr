...
//-->
					...
					<div class="tableStyle1">
						...
						<?
							...
							$i=1;
							$Rs=db_query("select keyword, count(idx) as cnt from tbl_counter $where group by keyword order by cnt desc");
							$total=db_result("select count(idx) from tbl_counter $where");
							while($pList=db_fetch($Rs)){
								$gwidth=1000*$pList['cnt']/$total;
								$percent=$pList['cnt']/$total*100;
						?>
							<tr>
								<td><?=$i?></td>
								<td title="<?=$pList['keyword']?>"><?=$pList['keyword']?></td>
								<td><?=$pList['cnt']?></td>
								<td><?=round($percent)?>%</td>
